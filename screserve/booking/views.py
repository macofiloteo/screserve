...
def create_booking(request):
    ...
    if admin_check(request.user):
        ...
        officers = Profile.objects.filter(user = request.user)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if not admin_check(request.user):
                form.cleaned_data["typeofreservation"]=booking_query.typeofreservation
                form.cleaned_data["dateofreservation"]=booking_query.dateofreservation
            form.save()
            return redirect('list_bookings')
        else:
        ...
        form = ReservationForm()
        if not admin_check(request.user):
            officer = Profile.objects.get(user = request.user)
        else:
            officer = Profile.objects.filter(user__is_active=True)
        return render(request, 'booking/updatereservation.html', {'officer':officer, 'booking':booking, 'form':form, 'history':history, 'group_date': group_date})
        ...
        if request.is_ajax():
            ...
            if (request.POST.get("booking_status")=='A'):
                if(bookToBeUpdated.turn == 0):
                    reservation_in_a_day = Reservation.objects.filter(isdeleted = False, dateofactualvisit = bookToBeUpdated.dateofactualvisit, typeofreservation = bookToBeUpdated.typeofreservation)
